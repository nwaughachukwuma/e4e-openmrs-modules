...
public class ConceptSearchHandler1_8 implements SearchHandler {
	...
	private final SearchConfig searchConfig = new SearchConfig("default", RestConstants.VERSION_1 + "/concept", Arrays.asList("1.8.*", "1.9.*", "1.10.*", "1.11.*", "1.12.*", "2.0.*", "2.1.*"),
	        Arrays.asList(
					...
					new SearchQuery.Builder("Allows you to find concepts by name and class")
							.withRequiredParameters("name")
							.withOptionalParameters("class", "searchType")
							.build()
			));
	...
	public PageableResult search(RequestContext context) throws ResponseException {
		...
		if ("fuzzy".equals(searchType)) {
			...
			List<ConceptSearchResult> searchResults = conceptService
					.getConcepts(name, locales, context.getIncludeAll(), classes, null, null, null, null, context.getStartIndex(), context.getLimit());
